...
#define APDEFAULT_CLOSEBUTTON_COLOR [UIColor colorWithRed:0.909804 green:0.913725 blue:0.909804 alpha:0.7F]
#define APDEFAULT_MAX_ALERTS_NUMBER 3
#define APDEFAULT_HIDING_DELAY 4
#define INFOVIEWHEIGT 90
#define INFOVIEW_X_POSITION 10
#define INFOVIEW_X_ENDMINUS 20
#define APPOSITIONFORALERT(i) (_APAlertPosition == APPlainAlertPositionBottom)?screenSize.height - INFOVIEWHEIGT * (i + 1) - 2 * (i):INFOVIEWHEIGT * (i) + 2 * (i)
...
static UIColor * _APPcloseButtonColor = nil;
...
+ (instancetype)showAlertWithTitle:(NSString *)title message:(NSString *)message type:(APPlainAlertType)type {
    if (!currentAlertArray) {
        currentAlertArray = [NSMutableArray new];  // Ha még nem inicializáltuk, inicializáljuk
    }

    // Ellenőrizzük a title és message értékeket
    if (title == nil) {
        title = @"Default Title"; // Alapértelmezett cím
    }
    if (message == nil) {
        message = @"No message provided."; // Alapértelmezett üzenet
    }

    // Az alert objektum létrehozása
    APPlainAlert *alert = [[APPlainAlert alloc] initWithTitle:title message:message type:type];
    [alert show];  // Megjelenítjük az alertet
    return alert;
}
...
- (void)viewDidLoad
{
    ...
    if (!_APPcloseButtonColor)
    {
        _APPcloseButtonColor = APDEFAULT_CLOSEBUTTON_COLOR;
    }
...
- (void)constructAlert
{
   ...
   // infoView.clipsToBounds = YES;
    ...
    UILabel * titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(60, 0, infoView.frame.size.width - INFOVIEWHEIGT, 70)];
    ...
    progressstitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(60, 57, infoView.frame.size.width - INFOVIEWHEIGT, 17)];
    ...
    {
       // UIImageView * closeView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"alert_close"]]; xmark.circle.fill
        UIImageView * closeView = [[UIImageView alloc] initWithImage:[UIImage systemImageNamed:@"xmark.circle.fill"]];
        closeView.tintColor=_closebuttonColor ? _closebuttonColor : _APPcloseButtonColor;;
        closeView.frame = CGRectMake(infoView.bounds.size.width - 42, 10, 11, 11);
        ...
        [infoView addSubview:closeView];
          ...
          NSLayoutConstraint *right = [NSLayoutConstraint constraintWithItem:closeView
           attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:infoView attribute:NSLayoutAttributeRight multiplier:1 constant:-15];
        NSLayoutConstraint *top = [NSLayoutConstraint constraintWithItem:closeView
         attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:infoView attribute:NSLayoutAttributeTop multiplier:1 constant:10];
    ...
    if (progressenabled) {
        self.progressBar=[[UIProgressView alloc] initWithFrame:CGRectMake(infoView.frame.origin.x+10, infoView.frame.origin.y+30, infoView.frame.size.width-X_POSITION_ENDMINUS-20, 2)];
...
- (void)showInMain
{
    @synchronized (currentAlertArray) {
        ...
        if (numberOfAlerts == 0) {
            ...
            [window insertSubview:self.view belowSubview:[((APPlainAlert *)[currentAlertArray lastObject]) view]];
